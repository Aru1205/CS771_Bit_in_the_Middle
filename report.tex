...
Table~\ref{tab:results} and Figure~\ref{fig:conv} summarize a representative
run.\footnote{Numbers were obtained on an 8000/2000 train/test split generated exactly as
described in the assignment (a hidden random 16-bit arbiter PUF generates the middle bit,
which is inserted and fed to a random 17-bit arbiter PUF, and then erased). Re-running
\texttt{evaluate.py} on the released public dataset reproduces the table for that data.}
...
\begin{enumerate}
  ...
  (normalized) weight vectors was $\approx -0.18$ ($\approx +0.23$ even after accounting for
  the flip symmetry described below) --- essentially unrelated directions. Norms differ as
  well, which is expected: on (near-)separable completed data the logistic MLE direction is
  what matters while the scale is set by the regularizer, so at best the directions could
  agree.
  \item[(b)] \textbf{No, $b \not\approx \tilde b$} ($b \approx 0.22$ vs
  $\tilde b \approx 0.98$ in our run).
  \item[(c)] The fraction of train CRPs with
  $2 z_i - 1 = \sign(\tilde\vu^\top \phi(\vc_i) + \tilde a)$ was $\approx 0.44$ ---
  \emph{chance level}. Indeed, $\{z_i\}$ agreed with the ground-truth middle bits on only
  $\approx 56\%$ of CRPs (up to a global flip), whereas the bits predicted by
  $(\tilde\vu, \tilde a)$ agreed with the ground truth on $\approx 99.6\%$.
\end{enumerate}

\begin{table}[t]
  \caption{Representative results (single machine, \texttt{sklearn} LBFGS solver).}
  \label{tab:results}
  \centering
  \begin{tabular}{lccc}
    \toprule
    ...
    \midrule
    \texttt{my\_latent()} (own latents) & $0.02$ s & $1.0000$ & --- \\
    \texttt{my\_latent\_updated()} (predicted latents) & $2.0$ s & $0.9978$ & $0.9975$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
fraction of $0.44$ is near $\nicefrac12$, i.e., the disagreement is real, not a flip artifact.
...
training loss to (near) zero from almost any initialization --- our runs reach $100\%$
training accuracy within a handful of alternations --- so there is a huge set of near-global
optima, and hard-EM simply lands on whichever one its initialization is attracted to. Nothing
in the objective rewards recovering the \emph{true} middle bits, and empirically the recovered
$\{z_i\}$ are at chance level with respect to them. This is classic overfitting through latent
variables: WTA lets each point ``explain away'' its response.
...
responses now forces the algorithm towards the true latent structure: the learnt
$(\tilde\vu, \tilde a)$ reproduces the ground-truth middle bits on $\approx 99.6\%$ of CRPs
(up to flip) and the pipeline generalizes to unseen test CRPs at $\approx 99.8\%$ --- which the
Part~1 model, whose latents are undefined for new points, cannot even attempt.
